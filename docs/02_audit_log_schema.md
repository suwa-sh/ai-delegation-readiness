...
【観測事実】事例記事の④統制・追跡層に明示されている最小要件です。
...
対応します。エージェント単独の Who(人間欠落)は最小段階で既に不可です。

## The recommended extensions (B): J-SOX グレード

【設計提案】規制が強い領域(J-SOX 対象会社・会計領域・金融業界)では、A に加えて
以下を必須に格上げします。事例記事には明示されていない、本リポでの一般化です。

| 拡張 | 内容 | 理由 |
|---|---|---|
| **規定バージョン** | Why の参照規定に **版(`version`)と条番号(`section`)** を必須化 | 規定改定後に過去判定を遡及検証するには版固定が不可欠です。J-SOX 観点で「いつの規定で判定したか」を説明できないと、統制有効性を主張できません |
| **Result の離散 enum** | Result を `approved` / `rejected` / `escalated` の **3 値の離散 enum** に強制 | 集計・監査クエリの信頼性を担保します。自由テキストでは「approved に見せかけた pending」を検出できません |
| **エスカレーション先の必須化** | `decision = escalated` のとき `escalated_to`(人間 ID)を必須に | 責任の所在をログ単独で追える形にします |

JSON Schema 上は [`$defs/audit_log_extended`](../schemas/audit-log.schema.json) に
対応します。`audit_log_minimum` を `allOf` で内包しつつ、Why と Result を厳格化
しています。
...
## Extension roadmap(本リポでは方向のみ提示、実装は提供しません)

【設計提案】会計・監査用途では、一部が実質 MVP 前提に近いものです。
...
承認業務のログは **後から集計・監査される**ことが前提です。連続値や自由テキスト
では、次の問題が起こります。
...
そのため、離散 3 値(`approved` / `rejected` / `escalated`)を CHECK 制約相当の
スキーマで強制します。
