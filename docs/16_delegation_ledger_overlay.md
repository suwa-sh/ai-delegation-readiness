...
- **成果あたりコスト** = AI への支出を、採用された成果(マージ・受入済みの成果物)の数で
  割った値。支出の総額だけでは委任の経済を説明できないため、分母の設計が問いになります
...
運用のままです。担当欄は 1 本で、エージェントに任せたのか誰が責任者なのかを読み分けられず、
コストは請求書の月次総額を委任件数で割った参考値しかありません。
...
| LA3 責任者起点の一覧出力の実績 | **no**(出せるはず、のまま) | yes(スクリプトで一覧を出力し件数を照合) |
| LA4 名目化に気づく観測 | **no**(観測なし) | **no**(観測なし。次の整備項目) |
| LC1 支出の抽出・集計 | yes(請求書明細のエクスポートあり) | yes(ツール横断で月次集計) |
| LC2 成果分母での計測 | **no**(月次総額 ÷ 委任件数) | yes(同一期間・同一範囲の支出と受入済み成果を対応づけ) |
| LC3 課金条件変化の統制 | yes(確認の責任者と周期あり) | yes(同左) |
...
時間系の指標を精査の proxy から除外し、指摘件数・修正要求率・抜き取り再検査という
能動的な指標を要求します。
...
| LA3 責任者起点の一覧を実際に出力したか | フィールドで検索できるはず | **no**(出力も照合もしていない) |
...
1. **LA1 / LA2 記録の分離と標準化**から始める。実行主体と説明責任者の 2 項目を
   記録の標準に足す。専用フィールドが無ければ説明文テンプレートでよい
2. **LA3 取り出しの実証**。責任者起点の一覧を実際に 1 回出力し、漏れを照合する。
   「出せるはず」を実績に変えるのがこの一歩です
3. **LC1 / LC2 集計と分母**。支出をツール横断で 1 回集計し、同一期間・同一範囲の
   採用成果と対応づける。厳密な個別原価でなくてよく、配賦のしかたと限界を記録します
4. **LA4 / LC3 劣化の検知**。承認の実態を能動的指標で観測する運用と、課金条件を
   確認する責任者・周期を決める。ここまで揃うと台帳は「劣化に気づける器」になります
