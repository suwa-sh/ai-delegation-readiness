...
「他人の DB スキーマに同じ点検手法を当てる」ための **worked example**(具体的な
適用例)です。ある **自社運用のエージェント基盤**の SQL スキーマを
Who/When/What/Why/Result の 5 観点で点検した結果と、`ALTER TABLE` での改修案を
...
## Worked example: 自社運用エージェント基盤のスキーマ

### 観測の前提

**【観測事実】** ある自社運用のエージェント基盤の SQL スキーマの列定義です。対象は
**承認・判定を伴う業務エージェント**(経理処理・取引先登録などを担うエージェント)
を想定しています。

**【設計提案】** 本書での改修案です(既存コードは触らず、別 issue 化する想定です)。
他の docs と同じく、観測事実と設計提案を本文中で明示的にラベル分けしています。

この基盤は記録の責務を 5 テーブルに分けています: `agent_runs` / `agent_costs` /
...
## なぜこの基盤が現状こうなっているか(背景)

この基盤は **コンテンツ自動化プラットフォーム**として設計されており、主要エージェントは
...
経理処理のような **承認・判定を伴う業務エージェント**を本格運用する段階で、本書の
改修が必要になります。これはこの基盤の設計が悪いわけではなく、**スコープ拡張に
伴う統制層の補強**という話です。
