...
10. **Extend (optional)** — add your own questions / thresholds via an overlay,
   ...
   authorization design ([docs/10](docs/10_agent_authorization_overlay.md)), and
...
└── docs/                        # Explanations, Japanese (reading order: the learning path in README.ja.md)
    ...
    ├── 07-10, 14                #   Applied: log-platform check / high-stakes domains / insourcing / agent authorization / unattended execution
...
- **AI agents**: load `definitions/*.yaml` and
  ...
  Code skill wrappers.
- **CI pipelines**: run `docker run --rm -v "$PWD:/data" -w /data ghcr.io/suwa-sh/ai-delegation-readiness:v0.15.1 validate-audit-log <log>` on each emitted log; gate
  on exit code.
