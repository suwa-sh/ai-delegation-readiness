...
> **release pending**: `v0.17.1` is waiting for its tag, Release, and image.
> Until they are published, use `v0.17.0` (the per-row answer rationales in
> the sample CSVs ship from `v0.17.1`).
...
- **AI agents**: load `definitions/*.yaml` and
  ...
  Code skill wrappers.
- **CI pipelines**: run `docker run --rm -v "$PWD:/data" -w /data ghcr.io/suwa-sh/ai-delegation-readiness:v0.17.1 validate-audit-log <log>` on each emitted log; gate
  on exit code.
