...
サンプル CSV のメモ列は、次の環境設定を前提に「何がどこにあり、何が欠けているか」を
描写します。実在ツール名は具体感のための例であり、推奨ではありません。

| 領域 | 設定 |
|---|---|
| 規定集 | 社内ポータルで版管理(改定日・理由・履歴)。経費規程は年 1 回改定 |
| SOP / 手順書 | 社内 wiki に集約。初回診断時点では粒度が粗くベテランの補足前提 |
| 経費精算 | ワークフロー SaaS で申請・承認。承認履歴と添付領収書が残り、確定データは経費 DB に保存 |
| 判定エンジン | ルールエンジン + LLM 補助の経費チェックエージェント(社内実装) |
| 監査ログ | ELK Stack に JSON で集約 |
| チケット / 委任台帳 | Redmine。AI 委任の管理方式は構成比較サンプルで異なる(担当者欄 1 本のままの構成と、説明文テンプレート + 夜間抽出スクリプトの構成) |
| コード / CI | GitLab + GitLab CI。テストカバレッジ計測あり |
| チャット | Slack。試行期の AI 依頼はここに散在していた |
| AI 利用 | 全社の社内 AI チャット(RAG 付き)+ 開発者の Copilot + 部門ごとのエージェント委任 |
| コスト | 各 SaaS / API の請求明細を月次エクスポートし、スプレッドシートで集計 |
| 実行基盤 | 情報システム部門の夜間バッチ worker(コンテナ)。egress は社内プロキシ経由 |
...
- **各サンプル CSV のメモ列には「このサンプルのシナリオではこういう状況だから yes / no」という回答理由を記載しています**(ミドリ精機の物語のサンプルはミドリ精機の状況、応用例の `ajinomoto-discovery-team.csv` は探索チームのシナリオの状況)。
...
### メモ列の書き方(サンプルを追加・変更する人向けの規約)

新しい overlay やサンプル CSV を追加するときは、全回答行のメモを次の規約で書きます。

1. **状況の描写だけを書く**。どのシステム・運用に何があり、何が欠けているかを、上の
   「システム環境」表の道具立てで描写する(表に無い道具が必要なら表に追記してから使う)
2. **問いの言い換えを書かない**。「〜だから yes / no」の定型サフィックスも付けない
   (描写から判定が読めるのが合格線)
3. **部分充足を理想形とする**。手本:
   「ELK Stack に監査ログも蓄積するルール。Who/When/What/Result は規定にあるが、Why が取れていない」(回答 no)。
   各メモに固有名詞または数値を最低 1 つ、長さは 25〜90 字目安
4. **ペアの対応を作る**。before/after・構成比較では、劣る側の no =「何がどう欠けているか」、
   良い側の yes =「同じ道具立てで欠落が埋まった状態」(before/after なら「改善: 」を前置)。
   共通の base 質問はペア間で同一文にする

メモは CLI が読まない自由記入欄なので判定には影響しませんが、回答・質問列は
`tests/test_init_input.py` のドリフト検査と verdict 凍結テストが守っています。
書き終えたら pytest と対象コマンドの実行で判定不変を確認してください。
