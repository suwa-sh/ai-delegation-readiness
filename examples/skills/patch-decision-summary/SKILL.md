...
4. JSON をユーザー向けに翻訳する。
   ...
   - gate cross-check の `RED accepted` は矛盾として明示する。`YELLOW accepted` は
     設計どおりの経路(人間が判断した)であり、異常ではない
...
- exit 2: RED を accepted した記録が 1 件以上ある。該当 `patch_id` を明示する(exit 1 より優先)
